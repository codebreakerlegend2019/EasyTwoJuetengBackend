...
namespace EasyTwoJuetengBackend.Helpers
{
    public class SqlControls : IDisposable
    {
        ...
        public SqlControls(string ConnectionString)
        {
        ...
        public void UpdateRange<T>(string table, List<T> entity) where T : class
        {
            ...
            try
            {
                ...
                using (var bulkcopy = new SqlBulkCopy(con))
                {
                    ...
                    bulkcopy.WriteToServer(SqlToolsHelper.ToDataTable(entity));
                    bulkcopy.Close();
                }
                columns = "";
        ...
        public List<TSource> MultiSelectData<TSource>(string query)
            where TSource : new()
        {
            var con = new SqlConnection(_connectionString);
            try
        ...
        public void Add<T>(string table, T entity) where T : class
        {
            try
            {
                using (var con = new SqlConnection(_connectionString))
                {
                    if (con.State != ConnectionState.Open)
                        con.Open();
                    var dataTable = SqlToolsHelper.ToDataTable(entity);
